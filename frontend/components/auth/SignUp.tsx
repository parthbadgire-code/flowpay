...
import { Eye, EyeOff, CheckCircle2, XCircle, UserPlus } from 'lucide-react';
import { supabase } from '@/lib/supabase';

// ─── Types ───────────────────────────────────────────────────────────────────

interface SignUpProps {
  /** Called after a successful sign-up when session is immediately available. */
  onSuccess?: () => void;
}
    ...
    >
      {checks.map(c => (
        <div key={c.label} className="flex items-center gap-2">
          ...
          <span className={`text-xs ${c.pass ? 'text-emerald-400' : 'text-slate-600'}`}>
            {c.label}
          </span>
        </div>
      ))}
  ...
  return (
    ...
    >
      <AnimatePresence mode="wait">
        ...
        {/* ── Success: immediately redirecting ── */}
        {status === 'redirecting' && (
          <motion.div
            key="redirecting"
            initial={{ opacity: 0, scale: 0.9 }}
          ...
          >
            <div
              className="w-20 h-20 rounded-full flex items-center justify-center mx-auto mb-5"
              style={{
                background: 'linear-gradient(135deg, rgba(124,110,255,0.2), rgba(107,92,231,0.1))',
                border: '2px solid rgba(124,110,255,0.4)',
              }}
            >
              <CheckCircle2 className="w-10 h-10" style={{ color: '#A99BFF' }} />
            </div>
        ...
        {status === 'idle' && (
          ...
          >
            <div className="flex items-center justify-center gap-2 mb-1">
              <UserPlus className="w-5 h-5" style={{ color: '#A99BFF' }} />
            ...
            {error && (
              <motion.p
                ...
                className="text-xs text-red-400 text-center mb-4 bg-red-500/10 rounded-xl py-2 px-3 border border-red-500/20"
              >
                {error}
              </motion.p>
            )}
